...
function titleSelect(e){
    var divNum = parseInt(e.target.parentNode.dataset.movie);
    changeFocus(divNum);
    setButtonsInvalid(divNum);
}

// a megfelelő film divjét kiegészíti a .focus osztállyal, a többiről pedig előbb  elveszi
function changeFocus(id){
    var cards = itemContainer.children;
    for (var i = 0; i < cards.length; i++){
        cards[i].classList.remove('focus');
        cards[i].classList.remove('focusPre');
        cards[i].classList.remove('focusPost');
    }
    cards[id].classList.add('focus');
    //hozzaadja a .focusPre class-t az elozo, a .focusPost class-t a kovetkezo elemhez
    if (id > 0) {cards[id-1].classList.add('focusPre')};
    if (id < cards.length-1) {cards[id+1].classList.add('focusPost')};
}
...
function setButtonsInvalid(id){
    ...
    else if (id === (maxCards - 1)){
        rightBtn.classList.add('invalid');
        leftBtn.classList.remove('invalid');        
    }
    else {
        rightBtn.classList.remove('invalid');
        leftBtn.classList.remove('invalid'); 
